...
- [ ] 各 VisualDef (SO) がエディタから作成・編集可能であること。
- [ ] タイル属性やエンティティスプライトがコードではなくSOから割り当てられていること。
- [ ] ゲームを再生した際、既存の（あるいは新しい）ビジュアルがエラーなく適用されること。
- [ ] `EntitySpriteFactory` のようなハードコードスプライト生成に依存していないこと。
