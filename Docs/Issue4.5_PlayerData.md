...
- [ ] `Player.prefab` を作成できること。
- [ ] シーンから `PlayerObj` を削除した状態で、指定した座標にプレイヤーがPrefabから生成されること。
- [ ] 生成されたプレイヤーにカメラが追従すること。
- [ ] シナリオやターン進行などの既存システムがエラーなく動作すること。
